...
namespace MothsEngine
{
	...
	private:
		...
		inline std::string GetAiMeshMaterialName(aiMesh* mesh)
		{
		...
		inline void ProcessMaterials(Model* model)
		{
			for (int i = 0; i < _scene->mNumMaterials; i++)
			{
				...
				Material* m = new Material(materialName.C_Str(), *Assets::LoadInternal<ShaderAsset>("data/shaders/lit.shader").GetShader());

				aiColor4D diffuse;
				if (AI_SUCCESS == aiGetMaterialColor(material, AI_MATKEY_COLOR_DIFFUSE, &diffuse))
				{
					m->SetColor("tintColor", Color(diffuse.r, diffuse.g, diffuse.b, diffuse.a));
				}

				model->materials.emplace(materialName.C_Str(), m);
			}
		}
		...
		void ProcessNode(Model* model, aiNode* node, std::vector<Mesh*>* meshes, Matrix4x4 correction)
		{
			if (node->mNumMeshes > 0)
			{
				...
				for (unsigned int i = 0; i < node->mNumMeshes; i++)
				{
					...
					mesh->AddMaterial(*(model->materials)[GetAiMeshMaterialName(aMesh)]);
