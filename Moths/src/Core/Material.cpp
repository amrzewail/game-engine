...
Material::Material(const std::string name, const Shader& shader) : name(name), shader(shader)
{
	_textures.clear();
	_assignedColors.clear();
	_assignedColors.emplace("tintColor", Color(1, 1, 1, 1));
}
...
void Material::Activate() const
{
	...
	if (_textures.size() == 0)
	{
		...
		Texture* white = Assets::LoadInternal<TextureAsset>("data/textures/white.png").GetTexture();
		glActiveTexture(GL_TEXTURE0 + 0);
		if (!white->IsReadyToUse()) white->Bind();
		glBindTexture(GL_TEXTURE_2D, *white->bindId);
